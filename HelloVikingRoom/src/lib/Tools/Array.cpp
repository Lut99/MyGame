...
template <class T> void Array<T>::push_back(const T& elem) {
...
template <class T> void Array<T>::erase(size_t index) {
    // Check if in bounds
...
template <class T> void Array<T>::erase(size_t start_index, size_t stop_index) {
    // Check if in bounds
...
template <class T> void Array<T>::reserve(size_t new_size) {
...
template <class T> T& Array<T>::at(size_t index) {
...
template <class T> const T& Array<T>::at(size_t index) const {
    if (index >= this->length) { throw std::out_of_range("Index " + std::to_string(index) + " is out-of-bounds for Array with size " + std::to_string(this->length)); }
    return this->elements[index];
}
...
template <class T> T* const Array<T>::wdata(size_t new_size) {
...
template <class T> Array<T>& Array<T>::operator=(Array<T>&& other) {
